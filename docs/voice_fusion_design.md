...
**第四个真实 bug:观测量本身在原始 logits 上有系统性偏差(已定位、已修复、真实引擎验证进行
中)**:轨迹反馈控制器第一次真实引擎验证时,结果趋势是错的——λ 从 0 加到 0.1 再到 0.2,
...
偏差)。真实引擎上"这个修法之后 λ 扫描是否真的收敛到居中"的验证见"仍需真实引擎验证的项"。
...
9. **轨迹反馈控制器改用 `matched_logits` 观测量之后的真实引擎复现验证**:第一版(观测原始
   per-member logits)在真实引擎上跑出了方向错误的结果(λ 越大越偏向同一个音色,见"AR 滞后"
   一节第四个 bug),已经用离线闭环仿真(`fuse_group_logits`/`mean_log_likelihood_of_sampled_
   frame` 的真实实现,人工构造 sharp/flat 不对称的 sibling)复现了这个异常,并确认改用
   `matched_logits` 作为观测量之后,同样的仿真在 λ=0.05/0.1/0.2 全部保持 100% 居中——但这只
   证明了"仿真里那个理想化的双峰-注意力模型下修法有效",不是真实模型的每一步 logits 分布都
   符合那个仿真假设的形状。仍然需要用同样的 A/B 对照协议(0.5/0.5、8 次独立生成、按参考音色
   单独克隆的音高中位数做锚点)在真实 GPU 上重新跑一遍 λ=0/0.05/0.1/0.2 扫描,确认这次是否
   真的收敛到"≥5/8 居中"这个此前预注册的成功标准,而不是仿真验证通过就直接当作生产就绪。
